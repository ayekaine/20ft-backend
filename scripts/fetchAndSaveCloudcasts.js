// TODO: perhaps move paging logic to the httpRequest script
...
async function fetchCloudcasts(startDate, endDate, nextUrl = null) {
  ...
  try {
    ...
    console.error("Failed to fetch data:", error.message); // Log errors from httpRequest
...
async function saveCloudcastToStrapi(cloudcast) {
  ...
  const cloudcastData = {
    ...
    // user: { ...cloudcast.user }, not sure if needed, the data always the same, offloads requests timeout (was removed from the model)
  };
